...
t_cola eliminar_rec(t_cola,t_cola);
int main()
{
    t_cola lista=NULL, eliminado;
    char crear,eliminar;
    int valor;
    do
    {
    ...
    printf("\n desea eliminar el nodo ingresado? s/n\n");
        eliminar=getche();
        if(eliminar=='s')
            eliminado=eliminar_rec(lista,lista);
    imprime_inversa(lista);
    printf("\nse elimino la direccion: %d que tenia %d como valor",eliminado,eliminado->valor);
    free(eliminado);
    return 0;
}
...
t_cola eliminar_rec(t_cola p,t_cola ant)
{
    if(p->sig!=NULL)
        return eliminar_rec(p->sig,p);
    else
    {
    ant->sig=NULL;
    return p;
